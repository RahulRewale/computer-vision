...
class DataLoader():
	...
	def load_val_test_data(self, input_shape=(256, 256), val_dir=None, test_dir=None):
		...
		all_ds = K.preprocessing.image_dataset_from_directory(
					...
					image_size=input_shape, batch_size=16 #self.batch_size
				)
		...
		self.val_ds = self.val_ds.map(lambda x, y: (self.generate_crops(x), y))
		self.test_ds = self.test_ds.map(lambda x, y: (self.generate_crops(x), y))
...
if __name__ == "__main__":
	...
	for batch_x, batch_y in train_ds:
		if batch_x.shape != (128, 227, 227, 3):
			plt.imshow(batch_x[0])
			plt.title(np.argmax(batch_y[0]))
			plt.show()
			print(batch_x.shape)
			print(batch_y.shape)